    ...
    def test_calculate_tour_length_s_shape_routing(self):
        for i, batch in enumerate(test_batches):
            with self.subTest(batch=i):
                ...
                # Prepare the table
                column_width = 15
                table_data = [[item['id'], item['abs_x_position'], item['abs_y_position'], item['abs_z_position']] for item in sorted_batch]
                
                # Print headers
                print(f"{'Product ID':<{column_width}} {'Aisle No.':<{column_width}} {'Y Position':<{column_width}} {'Z Position':<{column_width}}")
                print("-" * 60)
